...
class WebAction:
    ...
    def get_recommend(self, data):
        ...
        if Type in ['MOV', 'TV', 'ALL']:
            ...
            res_list = self.__get_douban_wish(CurrentPage)
    ...
    def get_torrents(type='downloading'):
        ...
        MediaHander = Media()
        group = {}
        history = DbHelper().get_download_history(hash=list(map(lambda x: x.get("id"), torrents)))
    ...
    @staticmethod
    def __get_douban_wish(page=1):
        """
        查询豆瓣同步历史
        """
        ret_infos = []
        start = (page - 1) * 15
        douban_wish = DouBan().get_douban_wish(dtype='wish', start=start, wait=True)
        for result in douban_wish:
            ret_infos.append({
                "id": "DB:%s" % result.get('id'),
                "orgid": result.get('id'),
                'title': result.get('title'),
                'image': result.get('cover'),
            })
        return ret_infos
