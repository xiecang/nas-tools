...
class DoubanSync(_IPluginModule):
        ...
        with lock:
            ...
            for media in medias:
                ...
                try:
                    ...
                    mark_date = history.get("mark_date")
                        ...
                        if self._auto_search:
                            ...
                            if not self._auto_rss:
                                ...
                                if search_result:
                                    ...
                                    if not no_exists.get(media_info.tmdb_id):
                                        ...
                                        self.__update_history(media=media_info, mark_date=mark_date, state="DOWNLOADED")
                            else:
                                # 需要加订阅，则由订阅去搜索
                                self.info(
                                ...
                                if code != 0:
                                    ...
                                    # 订阅已存在
                                    if code == 9:
                                        self.__update_history(media=media_info, mark_date=mark_date, state="RSS")
                                else:
                                    # 插入为已RSS状态
                                    self.__update_history(media=media_info, mark_date=mark_date, state="RSS")
                        else:
                            ...
                            if self._auto_rss:
                                ...
                                if code != 0:
                                    ...
                                    # 订阅已存在
                                    if code == 9:
                                        self.__update_history(media=media, mark_date=mark_date, state="RSS")
                                else:
                                    # 插入为已RSS状态
                                    self.__update_history(media=media, mark_date=mark_date, state="RSS")
                            elif not history:
    ...
    def __get_douban_info(self, douban_ids):
        ...
        for doubanid, info in douban_ids.items():
            ...
            # 组装媒体信息
            if not douban_info:
                self.warn("【Douban】%s 未正确获取豆瓣详细信息，尝试使用网页获取" % doubanid)
                douban_info = self.douban.get_media_detail_from_web(doubanid)
                if not douban_info:
                    self.warn("【Douban】%s 无权限访问，需要配置豆瓣Cookie" % doubanid)
                    # 随机休眠
                    sleep(round(random.uniform(1, 5), 1))
                    continue
            media_type = MediaType.TV if douban_info.get("episodes_count") else MediaType.MOVIE
            self.info("【Douban】%s：%s %s".strip() % (media_type.value, douban_info.get("title"), douban_info.get("year")))
